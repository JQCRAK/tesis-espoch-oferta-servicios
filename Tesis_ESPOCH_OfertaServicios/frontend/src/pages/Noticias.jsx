...
const ImgBlur = ({ url, grad, h = 200, radius = '10px 10px 0 0', children }) => (
    <div style={{
        ...
        {url
            ...
            : <div style={{
                position: 'absolute', inset: 0,
                background: grad, filter: 'brightness(0.55)',
              }} />
        }
        <div style={{
    ...
    const cargar = useCallback(async (pag = 1) => {
        ...
        try {
            const [nRes, eRes] = await Promise.all([
                axios.get(`${API}/noticias`, { params: { estado: 'publicada', page: pag, limit: LIMIT } }),
                axios.get(`${API}/eventos`,  { params: { estado: 'vigente', limit: 20 } }),
            ]);
        ...
        <>
            {cargando ? (
                <div style={s.featCard}>
                    <div style={{ height: isMobile ? 180 : 260, background: '#efefef', borderRadius: '10px 10px 0 0' }} />
                ...
                <div style={s.featCard}
                    ...
                    tabIndex={0} role="button">
                    <ImgBlur url={imgUrl(principal.imagen)} grad={GRADS[0]} h={isMobile ? 180 : 260} radius="10px 10px 0 0">
                        {(() => {
                            const c = CAT_CLR[principal.categoria] || CAT_CLR.comunicado;
                            return (
                                <span style={{ position: 'absolute', top: 12, left: 14, ...s.badge, background: c.bg, color: c.color }}>
            ...
            <div style={{
                ...
                <div style={{
                    ...
                    <div style={s.colMain}>
                        ...
                        {/* Sidebar debajo del contenido en móvil/tablet */}
                        {(isMobile || isTablet) && (
                ...
                return (
                    <div style={s.overlay}
                        ...
                        <div style={{
                            ...
                            <ImgBlur url={url} grad={GRADS[0]} h={isMobile ? 160 : 210} radius={isMobile ? '14px 14px 0 0' : '14px 14px 0 0'}>
                                {tipo === 'noticia'
                                    ...
                                    : <span style={{ position: 'absolute', top: 12, left: 14, ...s.badge, background: bEst.bg, color: bEst.color }}>
                                        📅 {bEst.label}
                                      </span>
                                }
                            ...
                            <div style={s.modalBody}>
                                ...
                                {item.contenido &&
                                    item.contenido.split('\n')
                                        .map(pg => pg.trim()).filter(Boolean)
                                        .map((pg, i) => <p key={i} style={s.modalP}>{pg}</p>)
                                }
                            </div>
...
const s = {
    ...
    cuerpo:      { maxWidth: 1160, margin: '0 auto' },
