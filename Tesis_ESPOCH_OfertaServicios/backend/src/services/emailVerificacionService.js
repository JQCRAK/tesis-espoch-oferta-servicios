...
const enviarCodigoVerificacion = async ({ emailInstitucional, codigo, nombres }) => {
  console.log(`\n📧 [EmailVerificacion] Enviando a: ${emailInstitucional}`);
...
<body style="margin:0;padding:0;background:#f5f5f5;font-family:'Segoe UI',Roboto,Arial,sans-serif;">
    ...
    <tr><td align="center">
      <table width="560" cellpadding="0" cellspacing="0" style="background:white;border-radius:12px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.10);">
        <tr><td style="background:#BE1E2D;padding:28px 36px;text-align:center;">
          <h1 style="margin:0;color:white;font-size:1.5rem;font-weight:800;">🎓 Verificación de Acceso</h1>
        ...
        <tr><td style="padding:32px 36px;">
          ...
          <p style="margin:0 0 24px;font-size:0.88rem;color:#6c757d;line-height:1.6;">
            Utiliza el siguiente código para verificar tu cuenta en el <strong>Portal de Graduados de la ESPOCH</strong>.
          </p>
          <div style="background:#f0f7ff;border:2px solid #1565c0;border-radius:12px;padding:28px 24px;margin-bottom:24px;text-align:center;">
            <p style="margin:0 0 10px;font-size:0.75rem;font-weight:700;color:#1565c0;letter-spacing:0.8px;text-transform:uppercase;">Tu código de verificación</p>
            <p style="margin:0;font-size:2.2rem;font-weight:900;color:#1565c0;letter-spacing:8px;font-family:monospace;">${codigo}</p>
            <p style="margin:10px 0 0;font-size:0.72rem;color:#1565c0;">Este código expira en 15 minutos</p>
          </div>
  ...
  try {
    ...
    console.log('✅ [EmailVerificacion] ENVIADO:', data.id);
    return { exito: true, messageId: data.id };
  } catch (err) {
    console.error('❌ [EmailVerificacion] ERROR:', err.message);
    throw new Error(`No se pudo enviar el código de verificación a ${emailInstitucional}: ${err.message}`);
  }
};
